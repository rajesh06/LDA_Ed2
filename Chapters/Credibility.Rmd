...
`r HideExample('11.2.1','Show Example Solution')`

**Solution.** Referring to a standard normal distribution table, $y_p=\Phi^{-1}((p+1)/2)=\Phi^{-1}((0.95+1)/2)$=$\Phi^{-1}(0.975)=1.960$.  Using this value and $k=.05$ then $\lambda_{kp} = (y_p/k)^{2}=(1.960/0.05)^{2}=1,536.64$.  After rounding up the full credibility standard is 1,537.
...
`r HideExample('11.2.2','Show Example Solution')`
...
`r HideExample('11.2.4','Show Example Solution')`
...
where $X$ is the amount of a single loss. See the discussion on *collective risk models* in Section \@ref(S:AggLoss:CRM) for more discussion of this framework. 
...
`r HideExample('11.2.5','Show Example Solution')`
...
\noindent Multiplying through by $m$ returns the bounds for losses
...
`r HideExample('11.2.6','Show Example Solution')`
...
`r HideExample('11.2.7','Show Example Solution')`
...
## B&uuml;hlmann Credibility {#S:Cred:Buhlmann}
...
If a policyholder is randomly chosen from the class and there is no loss information about the risk then the expected loss is $\mu=\mathrm{E}(\mu(\theta))$ where the expectation is taken over all $\theta$'s in the class.  In this situation $Z=0$ and the expected loss is $\hat\mu(\theta)=\mu$ for the risk.  The quantity $\mu$ can also be written as $\mu=\mathrm{E}(X_j)$ or $\mu=\mathrm{E}(\bar{X})$ and is often called the overall mean or `r Gloss('collective mean')`.  Note that $\mathrm{E}(X_j)$ is evaluated with the `r Gloss('law of total expectation')`: $\mathrm{E}(X_j)=\mathrm{E}(\mathrm{E}[X_j|\theta])$.   



**Example `r chapnum`.3.1.** 
The number of claims $X$ for an insured in a class has a Poisson distribution with mean $\theta>0$.  The risk parameter $\theta$ is exponentially distributed within the class with `r Gloss('pdf')` $f(\theta)=e^{-\theta}$.  What is the expected number of claims for an insured chosen at random from the class?
...
`r HideExample('11.3.2','Show Example Solution')`

**Solution** The expected number of claims for a risk is $\mathrm{E}(N|\lambda$)=$\lambda$.  The expected value for a Pareto distributed random variable is $\mathrm{E}(X | \theta, \alpha$)=$\theta/(\alpha-1)$.  The expected value of the aggregate loss random variable $S=X_1+\cdots+X_N$ for a risk with parameters $\lambda$, $\alpha$, and $\theta$ is $\mathrm{E}(S)=\mathrm{E}(N)\mathrm{E}(X)$ = $\lambda\theta/(\alpha-1)$.  The expected aggregate loss for a risk of type A is $\mathrm{E}(S_{\textrm{A}}$)=(0.5)(1000)/(2-1)=500.  The expected aggregate loss for a risk selected at random from the population is $\mathrm{E}(S)$ = 0.5[(0.5)(1000)]+0.3[(1.0)(1500)]+0.2[(2.0)(2000)]=1500.
...
**Example `r chapnum`.3.3.** The number of claims $N$ in a year for a risk in a population has a Poisson distribution with mean $\lambda>0$.  The risk parameter $\lambda$ is uniformly distributed over the interval $(0,2)$.  Calculate the $EPV$ and $VHM$ for the population.

`r HideExample('11.3.3','Show Example Solution')`

**Solution.** Random variable $N$ is Poisson with parameter $\lambda$ so $\mathrm{Var}(N|\lambda)=\lambda$.  The Expected Value of the Process variance is $EPV=\mathrm{E}(\mathrm{Var}(N|\lambda))$ = $\mathrm{E}(\lambda)=\int_{0}^{2}\lambda \frac{1}{2} d\lambda=1$.  The Variance of the Hypothetical Means is $VHM=\mathrm{Var}(\mathrm{E}(N|\lambda))$ = $\mathrm{Var}(\lambda)=\mathrm{E}(\lambda^2)-(\mathrm{E}(\lambda))^2$ = $\int_{0}^{2}\lambda^2 \frac{1}{2} d\lambda-(1)^2=\frac{1}{3}$.
...
Use the law of total variance to show that $\mathrm{Var}(\bar{X})$ = $VHM + (EPV/n)$ and derive a formula for $Z$ in terms of $\bar{X}$.

`r HideExample('11.3.4','Show Example Solution')`

**Solution** The quantity $\mathrm{Var}(\bar{X})$ is called the unconditional variance or the total variance of $\bar{X}$.  The law of total variance, equation \@ref(eq:LawTotVar), says 

$$
\mathrm{Var}(\bar{X})=\textrm{E(Var}(\bar{X}|\theta))+\textrm{Var(E}(\bar{X}|\theta)). 
$$

In bullet (1) at the beginning of this section we showed $\mathrm{E}(\mathrm{Var}(\bar{X}|\theta))$ = $EPV/n$.  In the bullet (2), $\mathrm{Var}(\mathrm{E}(\bar{X}|\theta))=VHM$.  Reordering the right hand side gives $\mathrm{Var}(\bar{X})$ = $VHM +(EPV/n)$.  Another way to write the formula for credibility $Z$ is  $Z=\mathrm{Var}(\mathrm{E}(\bar{X}|\theta))/\mathrm{Var}(\bar{X})$.  This implies $(1-Z)=\mathrm{E}(\mathrm{Var}(\bar{X}|\theta))/\mathrm{Var}(\bar{X})$.
...
`r HideExample('11.3.5','Show Example Solution')`

**Solution** Because individual loss amounts $X$ are exponentially distributed, $\mathrm{E}(X| \beta)=\beta$ and $\mathrm{Var}(X| \beta)=\beta^2$.  For aggregate loss $S=X_1+\cdots+X_N$, the mean is $\mathrm{E}(S)=\mathrm{E}(N)\mathrm{E}(X)$ and process variance is $\mathrm{Var}(S)=\mathrm{E}(N)\mathrm{Var}(X)+[\mathrm{E}(X)]^2 \mathrm{Var}(N)$.  With Poisson frequency and exponentially distributed loss amounts, $\mathrm{E}(S| \lambda, \beta) = \lambda\beta$ and $\mathrm{Var}(S| \lambda, \beta$) = $\lambda\beta^2+\beta^2\lambda=2\lambda\beta^2$.  
...
Define $Y_{jk}$ to be the loss for  the $k^{th}$ vehicle in the fleet for year $j$.  Then, the total losses for the fleet in year $j$ are $Y_{j1}+\cdots+Y_{jm_j}$ where we are adding up the losses for each of the $m_j$ vehicles.  In the B&uuml;hlmann-Straub model it is assumed that random variables $Y_{jk}$ are *iid* across all vehicles and years for the policyholder.  With this assumption the means $\mathrm{E}(Y_{jk}|\theta)=\mu(\theta)$ and variances $\mathrm{Var}(Y_{jk}|\theta)=\sigma^2(\theta)$ are the same for all vehicles and years.  The quantity $\mu(\theta)$ is the expected loss and $\sigma^2(\theta)$ is the variance in the loss for one year for one vehicle for a policyholder with risk parameter $\theta$.
...
Prove that $\mathrm{Var}(\bar{X}|\theta)=\sigma^2(\theta)/m$ for a risk with risk parameter $\theta$.

`r HideExample('11.4.1','Show Example Solution')`
...
In Example 11.3.4, it was shown that $Z=\mathrm{Var}(\mathrm{E}(\bar{X}|\theta))/\mathrm{Var}(\bar{X})$ where $\bar{X}$ is the average loss for $n$ observations.  In equation \@ref(eq:bscred) the $\bar{X}$ is the average loss for $m$ exposures and the same $Z$ formula can be used:
...
The denominator was expanded using the `r Gloss('law of total variance')`.  As noted above $\mathrm{E}(\bar{X}|\theta)=\mu(\theta)$ so $\mathrm{Var}(\mathrm{E}(\bar{X}|\theta))=\mathrm{Var}(\mu(\theta))=VHM$.
  Because $\mathrm{Var}(\bar{X}|\theta)=\sigma^2(\theta)/m$ it follows that $\mathrm{E}(\mathrm{Var}(\bar{X}|\theta))=\mathrm{E}(\sigma^2(\theta))/m$ = $EPV/m$.  Making these substitutions and using a little algebra gives
...
**Example `r chapnum`.4.2.**  A commercial automobile policyholder had the following exposures and claims over a three-year period:
...
`r HideExample('11.4.2','Show Example Solution')`
...
## Bayesian Inference and B&uuml;hlmann Credibility {#S:Cred:BayesInf}
...
Section \@ref(S:MS:BayesInference) reviews `r Gloss('Bayesian inference')` and it is assumed that the reader is familiar with that material.  The reader is also advised to read the B&uuml;hlmann credibility Section \@ref(S:Cred:Buhlmann) in this chapter.  This section will compare Bayesian inference with B&uuml;hlmann credibility and show connections between the two models.
...
If the risk had $n$ losses $x_1,\ldots, x_n$ then $\mathrm{E}(\mu(\theta)|x_1,\ldots, x_n)$ is the conditional expectation of $\mu(\theta)$.  The B&uuml;hlmann credibility formula $\hat{\mu}(\theta)=Z\bar{X}+(1-Z)\mu$ is a linear function of $\bar{X}=(x_1+\cdots+x_n)/n$ used to estimate $\mathrm{E}(\mu(\theta)|x_1,\ldots,x_n)$.  
...
The posterior distribution comes from `r Gloss('Bayes theorem')`
...
This formula is slightly different from Chapter \@ref(ChapModelSelection) because parameter $\beta$ is multiplied by $\lambda$ in the exponential of the gamma *pdf* whereas in Chapter \@ref(ChapModelSelection) $\lambda$ is divided by parameter $\theta$.  We have chosen this form for the exponential to simplify the equation for the expected number of claims. 
...
The B&uuml;hlmann credibility estimate for $\mathrm{E}(p|x)$ is exactly as the same as the Bayesian estimate as demonstrated in the following example.
...
`r HideExample('11.5.1','Show Example Solution')`

**Solution** Define random variables $Y_j$ such that $Y_j=1$ if the $j^{th}$ coin toss is heads and $Y_j=0$ if tails for $j=1,\ldots, n$.  Random variables $Y_j$ are *iid* conditional on $p$ with Pr$[Y=1|p]=p$ and Pr$[Y=0|p]=1-p$  The number of heads in $n$ tosses can be represented by the random variable $X=Y_1+\cdots+Y_n$.  We want to estimate $p=E[Y_j]$ using B&uuml;hlmann credibility: $\hat{p} = Z\bar{Y} +(1-Z)\mu$.  The overall mean is $\mu=\mathrm{E}(\mathrm{E}(Y_j|p))$ = $\mathrm{E}(p)=\alpha/(\alpha+\beta)$.  The sample mean is $\bar{y}=x/n$.  The credibility is $Z=n/(n+K)$ and $K=EPV/VHM$.  With $\mathrm{Var}(Y_j|p)=p(1-p)$ it follows that $EPV = \mathrm{E}(\mathrm{Var}[Y_j|p])$ = $\mathrm{E}(p(1-p)$).  Because $\mathrm{E}(Y_j|p)=p$ then $VHM=\mathrm{Var}((\mathrm{E}(Y_j|p))=\mathrm{Var}(p)$.  For the beta distribution
...
Parameter $K=EPV/VHM$=$[\mathrm{E}(p)-\mathrm{E}(p^2)]/\mathrm{Var}(p)$.  With some algebra this reduces to $K=\alpha+\beta$.  The B&uuml;hlmann credibility-weighted estimate is
...
As demonstrated in the prior section, the B&uuml;hlmann credibility estimates for the gamma-Poisson and beta-binomial models exactly match the Bayesian analysis results.  The term `r Gloss('exact credibility')` is applied in these situations.  Exact credibility may occur if the probability distribution for $X_j$ is in the linear exponential family and the prior distribution is a conjugate prior.  Besides these two models, examples of exact credibility also include Gamma-Exponential and Normal-Normal models. 

It is also noteworthy that if the conditional mean $\mathrm{E}(\mu(\theta)|X_1,...,X_n)$ is linear in the past observations, then the B&uuml;hlmann credibility estimate will coincide with the Bayesian estimate. More information about exact credibility can be found in [@BuhlmannGisler2005], [@klugman2012], and [@Tse2009].
...
but Var($\bar{X}_i$) is not the $VHM$.  Using equation \@ref(eq:LawTotVar), the total variance formula or *unconditional variance* formula is
...
As discussed previously in Section \@ref(S:EPV-VHM-Z), $EPV/n$ = $\mathrm{E}(\mathrm{Var}[\bar{X}_i|\Theta=\theta_i])$ and using the above estimators gives an unbiased estimator for the $VHM$:
...
`r HideExample('11.6.1','Show Example Solution')`
...
`r HideExample('11.6.2','Show Example Solution')`
...
`r HideExample('11.6.3','Show Example Solution')`
...
`r HideExample('11.6.4','Show Example Solution')`
...
`r HideExample('11.6.5','Show Example Solution')`
...
An example from the nonparametric B&uuml;hlmann-Straub section had the following data for two risks.  Find the amount associated with the complement of credibility, $\hat{M}_X$, that will produce credibility-weighted estimates that are in balance.
...
`r HideExample('11.6.6','Show Example Solution')`
