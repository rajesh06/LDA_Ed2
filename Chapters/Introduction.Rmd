...
[Table 1.1]: ../docs/ChapIntro.html#tab:1.1
...
`r HideRCode('T:Frequency.1',"R Code for Frequency Table")`
...
[Table 1.2] summarizes the sample distribution of average severities from the 403 policyholders who made a claim; it shows that the average claim amount was 56,330 (all amounts are in U.S. Dollars). However, the average gives only a limited look at the distribution. More information can be gleaned from the summary statistics which show a very large claim in the amount of 12,920,000. Figure \@ref(fig:SeverityFig) provides further information about the distribution of sample claims, showing a distribution that is dominated by this single large claim so that the histogram is not very helpful. Even when removing the large claim, you will find a distribution that is skewed to the right. A generally accepted technique is to work with claims in logarithmic units especially for graphical purposes; the corresponding figure in the right-hand panel is much easier to interpret.

<a id=tab:1.2></a> 

[Table 1.2]: ../docs/ChapIntro.html#tab:1.2
...
```{r SeverityFig, echo=FALSE, fig.cap='(ref:SeverityFig)', out.width='80%', fig.asp=.75, fig.align='center'}
...
Specifically, [Table 1.3] shows that we now consider
...
[Table 1.3]: ../docs/ChapIntro.html#tab:1.3
...
[^1.1]: Note that the average severity in [Table 1.3] differs from that reported in [Table 1.2]. This is because the former includes policyholders with zero claims where as the latter does not. This is an important distinction that we will address in later portions of the text. 

`r HideRCode('CoverageBC.1',"R Code for Claims Summary by Policyholder")`
...
[Table 1.3] shows that the average claim varies over time,
...
For a different look at the 2006-2010 data, [Table 1.4]
...
[Table 1.4]: ../docs/ChapIntro.html#tab:1.4
...
`r HideRCode('DeductCov.1',"R Code for Summary of Claim Frequency and Severity, Deductibles, and Coverages")`
...
[Table 1.6]: ../docs/ChapIntro.html#tab:1.6
...
`r HideRCode('ClaimRateVar.1',"R Code for Claims Summary by Entity Type, Fire Class, and No Claim Credit")`
...
[Table 1.7] shows the claims experience by alarm credit.
...
tables do not examine multivariate effects; for example, [Table 1.6] ignores the effect of size (as we measure through
coverage amounts) that affect claims.

<a id=tab:1.7></a> 

[Table 1.7]: ../docs/ChapIntro.html#tab:1.7
...
`r HideRCode('RateAlarmCredit.1',"R Code for Claims Summary by Entity Type and Alarm Credit Category")`
...
fair. For example, [Table 1.6] suggests that schools have
...
either. For example, we saw in [Table 1.7] that had we
...
Although property insurance is typically a one-year contract, [Table 1.3] suggests that policyholders tend to renew; this is
...
[Table 1.6] shows that policyholders without a claim in the last
...
*a. Claim Frequency.* The travel data history is long and stable. This coverage began on 1 November 2006. [Table 1.8] shows the count of claims for years 2015-2019, inclusive. Produce a comparable table of claims frequency for the entire period. Comment on the unusual frequency surrounding the COVID pandemic.

<a id=tab:1.8></a> 

[Table 1.8]: ../docs/ChapIntro.html#tab:1.8

Table 1.8. **2015-2019 Travel Claims Frequency**

```{r TravelClaims1, echo = FALSE}
...
knitr::kable(tableTravelShort, align = "cccccccc", caption = NA)  

# TableGen1(TableData=tableTravelShort,
#          TextTitle=NA,
#          Align='c', Digits=0, ColumnSpec=1:5,
#          ColWidth = ColWidth5)
...
*c. Loss Distributions over Time.* There are `r length(TravelClaimsGT10$Incurred.Loss)` incurred losses in the dataset with all available years (yet omitting claims less than 10). Figure \@ref(fig:TravelClaims2) shows that the distribution of incurred losses is stable over the period 2015-2019, inclusive. Produce a comparable figure for the entire period. 


```{r TravelClaims2, echo = FALSE, fig.cap='**Distribution of Travel Losses by Year**', out.width='60%', fig.asp=0.6}
TravelClaimsGT10 <- subset(TravelClaims, Incurred.Loss  >= 10)
...
knitr::kable(t(tableGPAFreqShort), align = "cccccccc", caption = NA)
...
(ref:GPAClaim1) **Distribution of Incurred Claims 2015-2019**

```{r GPAClaim1, echo=FALSE, fig.cap='(ref:GPAClaim1)', out.width='80%', fig.asp=.6}
par(mfrow=c(1, 2))
...
*d. Loss Distributions over Time.*  There are `r length(GPAClaimsGT10$Incurred.Loss)` incurred losses. Figure \@ref(fig:GPALossTime) indicates that the incurred losses are stable over the period 2015-2019, inclusive. Produce a comparable figure for the entire period and comment on the stability of the distribution.

```{r GPALossTime, echo = FALSE, fig.cap='**Distribution of Group Personal Accident Losses by Year**', out.width='70%', fig.asp=0.6}
...
*e. Deductibles.* For each event, or series of events arising from the one originating cause, ANU bears the amount of the excess in respect of each and every insured vehicle, unless stated otherwise. The standard deductible (or excess) in the dataset is 1000. However, a cursory examination of the dataset shows tremendous variation by vehicle and over time. Replicate [Table 1.10] that shows, for each year, the number of claims with zero excess, positive excess less than 1000, an excess equal to 1000, and an excess greater than 1000.


<a id=tab:1.10></a> 

[Table 1.10]: ../docs/ChapIntro.html#tab:1.10

Table 1.10. **Motor Vehicle Excess by Year**


```{r TableExcess, echo = FALSE}
...
knitr::kable(TableOut, align = "cccccccc", caption = NA)
...
`r HideExample('MotorData.1',"Details of the Motor Claims Data")`
...
knitr::kable(summary(AutoClaims)[,-1]) 
#%>%
 # kableExtra::kable_classic(font = 8, html_font = "Cambria")

```


```{r echo=F, eval=F}
sumAuto<- t(summary(AutoClaimsGT10$Motor.Net.Incurred, digits = 0))
knitr::kable(sumAuto, align = "cccccccc", caption = "**Motor Vehicle Incurred Losses**")
...
- **Edward (Jed) Frees**, University of Wisconsin-Madison, is the principal author of the initial version of this chapter.
...
This book introduces loss data analytic tools that are most relevant to actuaries and other financial risk analysts. We have also introduced you to many new insurance terms; more terms can be found at the @NAICGlossary. Here are a few references cited in the chapter.
