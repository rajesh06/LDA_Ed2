...
# Loss Reserving {#ChapLossReserves}
...
(ref:tikz-run-off)  **Lifetime or Run-off of a Claim**

<a id="tikz-run-off"></a>
```{tikz, tikz-run-off, fig.cap = '(ref:tikz-run-off)', fig.align = 'center', fig.ext = 'png', fig.width=5, echo = FALSE, cache=TRUE}
...
(ref:tikz-closed)  **Lifetime of a Closed Claim**

<a id="tikz-closed"></a>
```{tikz, tikz-closed, fig.cap = '(ref:tikz-closed)', fig.align = 'center', fig.ext = 'png', fig.width=5, echo = FALSE, cache=TRUE}
\usetikzlibrary{snakes}
\begin{tikzpicture}[snake=zigzag, line before snake = 5mm, line after snake = 5mm]
%draw horizontal line
\draw[->] (3,0) -- (15,0);
...
(ref:tikz-RBNS)  **Lifetime of an RBNS Claim**

<a id="tikz-RBNS"></a>
```{tikz, tikz-RBNS, fig.cap = '(ref:tikz-RBNS)', fig.align = 'center', fig.ext = 'png', fig.width=5, echo = FALSE, cache=TRUE}
\usetikzlibrary{snakes}
...
(ref:tikz-IBNR)  **Lifetime of an IBNR Claim**

<a id="tikz-IBNR"></a>
```{tikz, tikz-IBNR, fig.cap = '(ref:tikz-IBNR)', fig.align = 'center', fig.ext = 'png', fig.width=5, echo = FALSE, cache=TRUE}
\usetikzlibrary{snakes}
\usetikzlibrary{shapes.arrows}
\definecolor{shadecolor}{HTML}{f67e8e}  %licht rood
\begin{tikzpicture}[scale=0.8,snake=zigzag, line before snake = 5mm, line after snake = 5mm]
...
(ref:tikz-micro-macro) **From Granular Data to Run-off Triangle**

<a id="tikz-micro-macro"></a>
```{tikz, tikz-micro-macro, fig.cap = '(ref:tikz-micro-macro)', fig.align = 'center', fig.ext = 'png', fig.width=5, echo = FALSE, cache=TRUE}
\tikzset{
triangle/.style={
...
(ref:tikz-triangle) **A Run-off Triangle with Incremental Payment Data.** *Source: @WuthrichMerz2008, Table 2.2.*

<a id="tikz-triangle"></a>
```{tikz tikz-triangle, fig.cap = '(ref:tikz-triangle)', fig.align = 'center', fig.ext = 'png', fig.width=10, echo = FALSE, cache=TRUE}
\begin{tiny}
\begin{center}
...
(ref:tikz-cum-triangle) **A Run-off Triangle with Cumulative Payment Data.** *Source: @WuthrichMerz2008, Table 2.2.*

<a id="tikz-cum-triangle"></a>
```{tikz tikz-cum-triangle, fig.cap = '(ref:tikz-cum-triangle)', fig.align = 'center', fig.ext = 'png', fig.width=10, echo = FALSE, cache=TRUE}
\begin{tiny}
\begin{center}
\begin{tabular}{|c||r r r r r r r r r r|}
\hline
...
* the change in incurred amounts, where incurred claim amounts are the sum of cumulative paid claims and the `r Gloss('case estimates')`. The case estimate is the claims handler’s expert estimate of the outstanding amount on a claim. 
...
(ref:tikz-math-triangle)  **Mathematical notation for a run-off triangle.** *Source:* @WuthrichMerz2008

<a id="tikz-math-triangle"></a>
```{tikz, tikz-math-triangle, fig.cap = '(ref:tikz-math-triangle)', fig.align = 'center', fig.ext = 'png', fig.width=10, echo = FALSE, cache=TRUE}
\begin{scriptsize}
\begin{center}
\begin{tabular}{|c||r r r r r r r r r r|}
\hline
...
(ref:plottriangle) **Claim Development by Occurrence Year**

```{r plottriangle, fig.cap = '(ref:plottriangle)', fig.align = 'center', out.width = '80%'}
plot(my_triangle)
```
...
Thus, the development factor tells you how the cumulative amount in development year $j$ grows to the cumulative amount in year $j+1$. We highlight the cumulative amount in period 0 in blue and the cumulative amount in period 1 in red on the Figure \@ref(fig:tikz-cum-triangle-cl1) taken from @WuthrichMerz2008 (Table 2.2, also used in @WuthrichMerz2015, Table 1.4).

(ref:tikz-cum-triangle-cl1) **A Run-off Triangle with Cumulative Payment Data Highlighting the Cumulative Amount in Period 0 in Blue and the Cumulative Amount in Period 1 in Red.** *Source: @WuthrichMerz2008, Table 2.2.*

<a id="tikz-cum-triangle-cl1"></a>
```{tikz, tikz-cum-triangle-cl1, fig.cap = '(ref:tikz-cum-triangle-cl1)', fig.align = 'center', fig.ext = 'png', fig.width=10, echo = FALSE, cache=TRUE}
\begin{tiny}
\begin{center}
\begin{tabular}{|c||r r r r r r r r r r|}
\hline
...
The chain-ladder method then presents an intuitive recipe to estimate or calculate these development factors. Since the first development factor $f_0$ describes the development of the cumulative claim amount from development period 0 to development period 1, it can be estimated as the ratio of the cumulative amounts in red and the cumulative amounts in blue, highlighted in the  Figure \@ref(fig:tikz-cum-triangle-cl1). We then obtain the following estimate $\hat{f}_0^{CL}$ for the first development factor $f_0$, given observations $\mathcal{D}_I$:
...
(ref:tikz-cum-triangle-cl2) **A Run-off Triangle with Cumulative Payment Data Highlighting the Cumulative Amount in Period 1 in Blue and the Cumulative Amount in Period 2 in Red.** *Source: @WuthrichMerz2008, Table 2.2.*

<a id="tikz-cum-triangle-cl2"></a>
```{tikz, tikz-cum-triangle-cl2, fig.cap = '(ref:tikz-cum-triangle-cl2)', fig.align = 'center', fig.ext = 'png', fig.width=10, echo = FALSE, cache=TRUE}
\begin{tiny}
\begin{center}
\begin{tabular}{|c||r r r r r r r r r r|}
\hline
...
(ref:tikz-cum-triangle-cl3) **A Run-off Triangle with Cumulative Payment Data Including Predictions in Italic** *Source: @WuthrichMerz2008, Table 2.2.*

<a id="tikz-cum-triangle-cl3"></a>
```{tikz, tikz-cum-triangle-cl3, fig.cap = '(ref:tikz-cum-triangle-cl3)', fig.align = 'center', fig.ext = 'png', fig.width=10, echo = FALSE, cache=TRUE}
\begin{tiny}
\begin{center}
\begin{tabular}{|c||r r r r r r r r r r|}
\hline
...
(ref:tikz-cum-triangle-cl3b) **A Run-off Triangle with Cumulative Payment Data Highlighting the Cumulative Amount in Period 8 in Blue and the Cumulative Amount in Period 9 in Red.** *Source: @WuthrichMerz2008, Table 2.2.*

<a id="tikz-cum-triangle-cl3b"></a>
```{tikz, tikz-cum-triangle-cl3b, fig.cap = '(ref:tikz-cum-triangle-cl3b)', fig.align = 'center', fig.ext = 'png', fig.width=10, echo = FALSE, cache=TRUE}
\begin{tiny}
\begin{center}
\begin{tabular}{|c||r r r r r r r r r r|}
\hline
...
(ref:tikz-cum-triangle-cl4) **A Run-off Triangle with Cumulative Payment Data Including Predictions in Italic** *Source: @WuthrichMerz2008, Table 2.2.*

<a id="tikz-cum-triangle-cl4"></a>
```{tikz, tikz-cum-triangle-cl4, fig.cap = '(ref:tikz-cum-triangle-cl4)', fig.align = 'center', fig.ext = 'png', fig.width=10, echo = FALSE, cache=TRUE}
\begin{tiny}
\begin{center}
\begin{tabular}{|c||r r r r r r r r r r|}
\hline
...
(ref:tikz-cum-triangle-cl5) **Reserves per Occurence Period and for Total**

<a id="tikz-table-cl5"></a>
```{tikz, tikz-cum-triangle-cl5, fig.cap = '(ref:tikz-cum-triangle-cl5)', fig.align = 'center', fig.ext = 'png', fig.width=10, echo = FALSE, cache=TRUE}
\begin{tiny}
\begin{center}
...
Third, the **gamma** regression model is relevant to model a run-off triangle with claim payments. Recall from Section \@ref(S:Loss:Gamma) (see also the Appendix Chapter \@ref(ChapSummaryDistributions)) that the gamma distribution has shape parameter $\alpha$ and scale parameter $\theta$. From these, we reparameterize and define a new parameter $\mu = \alpha \cdot \theta$ while retaining the scale parameter $\theta$. Further, assume that $X_{ij}$ has a gamma distribution and allow $\mu$ to vary by $ij$ such that 
...
To read more about exponential families and generalized linear models, see, for example, @McCullaghNelder89 and @WuthrichMerz2008. We refer to [@Kremer1982], [@RenshawVerrall1998] and [@EnglandVerrall2002], and the overviews in [@Taylor2000], [@WuthrichMerz2008] and [@WuthrichMerz2015] for more details on the discussed GLMs. XXX presents alternative distributional assumptions and specifications of the linear predictor. 
