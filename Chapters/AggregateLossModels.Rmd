...
**Example 6.2.1. Actuarial Exam Question.**
...
`r HideExample('6.2.1', 'Show Example Solution')`
...
`r HideExample('6.2.2','Show Example Solution')`
...
**Example 6.2.3. Actuarial Exam Question - Follow-Up.**
As in the Example 6.2.1 earlier, an insurance company sold 300 fire insurance policies, with claim amounts $X_i$ uniformly distributed between 0 and the policy maximum $M_i$. Using the normal approximation, calculate the probability that the aggregate claim amount $S_{300}$ exceeds $\$3,500$. 

`r HideExample('6.2.3','Show Example Solution')`
...
**Example 6.2.4. Gamma Distribution.**
...
**Example 6.2.5. Negative Binomial Distribution.** 
...
**Example 6.2.6. Actuarial Exam Question (modified).**
...
`r HideExample('6.2.6','Show Example Solution')`
...
**Example 6.3.1. Actuarial Exam Question.**
...
`r HideExample('6.3.1','Show Example Solution')`
...
**Example 6.3.2. Actuarial Exam Question.**
...
`r HideExample('6.3.2','Show Example Solution')`
...
**Example 6.3.3. Actuarial Exam Question.**
...
`r HideExample('6.3.3','Show Example Solution')`
...
**Example 6.3.4. Actuarial Exam Question.**
You are given:
...
`r HideExample('6.3.4','Show Example Solution')`
...
**Example 6.3.5. Actuarial Exam Question.**
...
`r HideExample('6.3.5','Show Example Solution')`
...
Recall the coverage modifications on the individual policy level in Section \@ref(S:CoverageModifications). Insurance on the aggregate loss $S_N$, subject to a `r Gloss('deductible')` $d$, is called \textit{net stop-loss insurance}. The expected value of the amount of the aggregate loss in excess of the deductible,
...
**Example 6.3.6. Actuarial Exam  Question.**
...
`r HideExample('6.3.6','Show Example Solution')`
...
**Example 6.3.7. Actuarial Exam  Question - Continued.**
...
\end{aligned}$$
Thus, the *mgf* of aggregate loss $S_N$ can be expressed two ways (for details, see *Technical Supplement 6.A.3*)
\begin{eqnarray}
...
From (6.1), we note that $S_N$ is equivalent to the compound distribution of $S_N=X^{*}_1+\cdots+X^{*}_{N^{*}}$, where $N^{*}$ is a Bernoulli with mean $\beta/(1+\beta)$ and $X^{*}$ is an exponential with mean $\theta(1+\beta)$. To see this, we examine the *mgf* of $S$:
$$\begin{aligned}
...
From (6.2), we note that $S_N$ is also equivalent to a two-point mixture of 0 and $X^{*}$. Specifically,
...
**Example 6.3.9.**
Consider a collective risk model with an exponential severity and an arbitrary frequency distribution. Recall that if $X_i\sim Exp(\theta)$, then the sum of *iid* exponential random variables, $S_n=X_1+\cdots+X_n$, has a gamma distribution, i.e. $S_n\sim Gam(n,\theta)$. This has cdf:
\begin{eqnarray*}
...
**Example 6.4.1. Actuarial Exam Question.**
...
`r HideExample('6.4.1','Show Example Solution')`
...
We now summarize simulation procedures for aggregate loss models. Let $m$ be the size of the generated random sample of aggregate losses.
...
where $I(\cdot)$ is an indicator function. The `r Gloss('empirical distribution')` $\hat{F}_S(s)$ will `r Gloss('converge')` to ${F}_S(s)$ almost surely as the sample size $m\rightarrow \infty$.
...
**Example 6.4.2.**
Recall Example 6.3.5 with an individual's claim frequency $N$ has a Poisson distribution with mean $\lambda=25$ and claim severity $X$ is uniformly distributed on the interval $(5,95)$. Using a simulated sample of 10,000 observations, estimate the mean and variance of the aggregate loss $S_N$. In addition, use the simulated sample to estimate the probability that aggregate claims for this individual will exceed 2,000 and compare with the normal approximation estimates from Example 6.3.5.

`r HideExample('6.4.2','Show Example Solution')`
...
`r HideExample('6.5.1','Show Example Solution')`
...
`r HideExample('6.5.2','Show Example Solution')`
...
`r HideExample('6.5.3','Show Example Solution')`
...
`r HideExample('6.5.4','Show Example Solution')`
...
**Example 6.5.5. Actuarial Exam Question.**
...
`r HideExample('6.5.5','Show Example Solution')`
...
**Example 6.5.6. Follow-Up.**
What is the variance of the total claim payment, $\mathrm{Var~}(S)$?  


`r HideExample('6.5.6','Show Example Solution')`
...
**Example 6.5.7. Actuarial Exam Question.**
...
`r HideExample('6.5.7','Show Example Solution')`
...
<!-- ##Technical Supplement 6.A. Aggregate Loss Models {-} -->

### TS 6.A.1. Individual Risk Model Properties {-}
...
### TS 6.A.2. Relationship Between Probability Generating Functions of $X_i$ and $X_i^T$ {-}
...
### TS 6.A.3. Example 6.3.8 Moment Generating Function of Aggregate Loss $S_N$ {-}
...
which gives the expression (6.1). For the alternate expression of the *mgf* (6.2), we continue from where we just left off:
