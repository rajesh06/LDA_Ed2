...
**Definition  `r chapnum`.1.**  Consider a non-negative loss random variable $X$.
...
<!-- \label{exm:gamma}   -->
**Example `r chapnum`.2.1. Light tail nature of the gamma distribution.**
...
**Example `r chapnum`.2.2. Light tail nature of the Weibull distribution.**
...
**Example `r chapnum`.2.3. Heavy tail nature of the Pareto distribution.**
...
**Definition  `r chapnum`.2.** For two random variables $X$ and $Y$, let
...
**Example `r chapnum`.2.4. Comparison of Pareto to Weibull distributions.**
...
**Example `r chapnum`.2.5. Comparison of Pareto to gamma distributions.**
...
```{r child = './Quizzes/Quiz102.html', eval = QUIZ}
```
...
$$
One can check that all the aforementioned functions are risk measures in which we input the loss random variable and the functions output a numerical value.  On a different note, the function $H^{\ast}(X)=\alpha X^{\beta}$ for any real-valued $\alpha,\beta\neq 0$, is not a risk measure because $H^{\ast}$ produces another random variable rather than a single numerical value.
...
To see that the standard deviation is not a coherent risk measure, start by checking that the standard deviation satisfies
...
**Special Case. The Standard Deviation Principle \@ref(eq:SD-principle) is a Coherent Risk Measure.**

`r HideExample('PortMgt.3.2', 'Show Verification of the Special Case')`
...
Here, $inf$ is the infimum operator so that the $VaR$ measure outputs the smallest value of $X$ such that the associated `r Gloss('cdf')` first exceeds or equates to $q$. This is simply the `r Gloss('quantile')` that was introduced in Section \@ref(S:LS:Quantiles) and further developed in Section \@ref(S:MS:QuantileEstimator).
...
**Example `r chapnum`.3.1. $VaR$ for the exponential distribution.**
Consider an insurance loss random variable $X$ with an exponential distribution having parameter $\theta$ for $\theta>0$, then the *cdf* of $X$ is given by 
$$
...
The result reported in Example `r chapnum`.3.1 can be generalized to any continuous random variables having a strictly increasing *cdf*.  Specifically, the $VaR$ of any continuous random variables is simply the inverse of the corresponding *cdf*.  Let us consider another example of continuous random variable which has the support from negative infinity to positive infinity.


**Example `r chapnum`.3.2. $VaR$ for the normal distribution.**
Consider an insurance loss random variable $X\sim Normal(\mu,\sigma^2)$ with $\sigma>0$.  In this case, one may interpret the negative values of $X$ as profit or revenue.  Give a closed-form expression for the $VaR$.
...
In many insurance applications, we have to deal with transformations of random variables.  For instance, in Example `r chapnum`.3.2, the loss random variable $X\sim Normal(\mu, \sigma^2)$ can be viewed as a linear transformation of a standard normal random variable $Z\sim Normal(0,1)$, namely $X=Z\sigma+\mu$.  By setting $\mu=0$ and $\sigma=1$, it is straightforward for us to check $VaR_q[Z]=\Phi^{-1}(q).$  A useful finding revealed from Example `r chapnum`.3.2 is that the $VaR$ of a linear transformation of the normal random variables is equivalent to the linear transformation of the $VaR$ of the original random variables.  This finding can be further generalized to any random variables as long as the transformations are strictly increasing. 

**Example `r chapnum`.3.3. $VaR$ for transformed variables.**
Consider an insurance loss random variable $Y$ with a lognormal distribution with parameters $\mu \in \mathbf{R}$ and $\sigma^2>0$. Give an expression of the $VaR$ of $Y$ in terms of the standard normal inverse *cdf*.

`r HideExample('12.3.3','Show Example Solution')`
...
<!-- \label{exm:discrete} -->
**Example `r chapnum`.3.4. $VaR$ for a discrete random variable.**
...
`r HideExample('12.3.4','Show Example Solution')`
...
* the selection of the confidence level $q\in (0,1)$ is highly subjective, while the $VaR$ can be very sensitive to the choice of $q$ (e.g., in Example `r chapnum`.3.4, $VaR_{0.95}[X]=3$ and $VaR_{0.950001}[X]=4$);
...
Consider two loss random variable's $X\sim Uniform [0,100]$, and $Y$ with an exponential distribution having parameter $\theta=31.71$.  We use $VaR$ at $95\%$ confidence level to measure the riskiness of $X$ and $Y$.  Simple calculation yields (see, also, Example `r chapnum`.3.1),
$$
...
**Example `r chapnum`.3.6. $TVaR$ for a normal distribution.**
Consider an insurance loss random variable\ $X\sim Normal (\mu,\sigma^2)$ with $\mu\in \mathbf{R}$ and $\sigma>0$. Give an expression for $TVaR$.

`r HideExample('12.3.6','Show Example Solution')`

**Solution.**

Let $Z$ be the standard normal random variable. For $q\in(0,1)$, the $TVaR$ of $X$ can be computed via
...
where `$\overset{(1)}{=}$' holds because of the results reported in Example `r chapnum`.3.2.  Next, we turn to study $TVaR_q[Z]=\mathrm{E}[Z|Z>VaR_q[Z]]$.  Let $\omega(q)=(\Phi^{-1}(q))^2/2$, we have
...
We mentioned earlier in the previous subsection that the $VaR$ of a strictly increasing function of random variable is equal to the function of $VaR$ of the original random variable.  Motivated by the results in Example `r chapnum`.3.6, one can show that the $TVaR$ of a strictly increasing linear transformation of random variable is equal to the function of $VaR$ of the original random variable. This is due to the linearity property of expectations.  However, the aforementioned  finding cannot be extended to non-linear functions.  The following example of lognormal random variable serves as a counter example.  

**Example `r chapnum`.3.7. $TVaR$ of a lognormal distribution.**
Consider an insurance loss random variable $X$ lognormal distribution with parameters $\mu \in \mathbf{R}$ and $\sigma>0$. Show that

$$
TVaR_q[X] = \frac{e^{\mu+\sigma^2/2}}{(1-q)} \Phi(\Phi^{-1}(q)-\sigma).
$$

`r HideExample('12.3.7','Show Example Solution')`
...
where $\overset{(1)}{=}$ holds by applying change of variable $w=(\log x-\mu)/\sigma$, and $\omega(q)=(\log \pi_q-\mu)/\sigma$.  Evoking the formula of $VaR$ for lognormal random variable reported in Example  `r chapnum`.3.2, we can simplify the expression \@ref(eq:cte-normal) into
...
**Example  `r chapnum`.3.8. $TVaR$ of an exponential distribution.**
Consider an insurance loss random variable $X$ with an exponential distribution having parameter $\theta$ for $\theta>0$. Give an expression for the $TVaR$.

`r HideExample('12.3.8','Show Example Solution')`
...
where $e_X(d)=\mathrm{E}[X-d|X>d]$ for $d>0$ denotes the `r Gloss('mean excess loss function')`. For many commonly used parametric distributions, the formulas for calculating $\mathrm{E}[X]$ and $\mathrm{E}[X\wedge\pi_q]$ can be found in a table of distributions.


**Example  `r chapnum`.3.9. $TVaR$ of a Pareto distribution.**
Consider a loss random variable $X\sim Pareto(\theta,\alpha)$ with $\theta>0$ and $\alpha>0$.  The *cdf* of $X$ is given by
$$
...
From Section \@ref(S:ContinuousDistributions), we know
$$
...
<!-- \label{def:TVaR}-->
**Definition  `r chapnum`.5.**
Fix $q\in (0,1)$, the  *conditional value at risk* of a random variable $X$ is formulated as

$$
CVaR_q[X] = \frac{1}{1-q}\int_{q}^{1} VaR_{\alpha}[X]\ d\alpha .
$$

The conditional value at risk is also known as the *average value at risk (AVaR)* and the *expected short-fall (ES)*. It can be shown that $CVaR_q[X] = TVaR_q[X]$ when $\Pr(X=VaR_q[X])=0$ which holds for continuous random variables. That, is, if $X$ is continuous, then via a change of variables, we can rewrite equation \@ref(eq:cte-pdf) as
...
```{r child = './Quizzes/Quiz103.html', eval = QUIZ}
```
...
Recall that `r Gloss('reinsurance', '1.1, 10.4')` is simply insurance purchased by an insurer. Insurance purchased by non-insurers is sometimes known as  `r Gloss('primary insurance')` to distinguish it from reinsurance. Reinsurance differs from personal insurance purchased by individuals, such as auto and homeowners insurance, in contract flexibility. Like insurance purchased by major corporations, reinsurance programs are generally tailored more closely to the buyer. For contrast, in personal insurance buyers typically cannot negotiate on the contract terms although they may have a variety of different options (contracts) from which to choose.
...
```{r comment="", echo=SHOW_PDF, message=FALSE, eval=FALSE, fig.width=10, fig.height=4, fig.align='center'}
set.seed(2018)
theta = 1000
alpha = 3
nSim = 10000
...
**Proposition**. Suppose that $Var(Y_{insurer})=Q.$ Then, $Var ((1-c)X) \le Var(g(X))$ for all $g(.)$ such that $\mathrm{E} [g(X)]=K$, where $c=Q/Var(X)$.
...
**Example `r chapnum`.4.2. Three Pareto risks.** Consider three risks that have a Pareto distribution, each having a different set of parameters (so they are independent but non-identical). Specifically, use the parameters:
...
```{r comment="", message=FALSE, echo=SHOW_PDF, fig.width=8, fig.height=4, fig.align='center'}
...
Taking a partial derivative of $L$ with respect to $\lambda$ and setting this equal to zero simply means that the constraint, $E(Y_{insurer}) = K$, is enforced and we have to choose the limits $M_i$ to satisfy this constraint. Moreover, taking the partial derivative with respect to each limit $M_i$ yields
...
**Example `r chapnum`.4.3. Excess of loss for three Pareto risks.** Consider three risks that have a Pareto distribution, each having a different set of parameters (so they are independent but non-identical). Use the same set of parameters as in Example `r chapnum`.4.2. For this example:

a.  Show numerically that the optimal retention limits $M_1$, $M_2$, and $M_3$ resulting retention limit minus expected insurer's claims, $M_i - E(X_i \wedge M_i)$, is the same for all risks, as we derived theoretically. 
...
```{r comment="", echo=SHOW_PDF, message=FALSE, warning=FALSE, fig.width=8, fig.height=4, fig.align='center'}
...
```{r comment="", message=FALSE, warning=FALSE, fig.width=8, fig.height=4, fig.align='center', echo=SHOW_PDF}
...
**Example `r chapnum`.4.4. Layers of coverage for three parties.**
...
#### Portfolio Management Example {#S:PortMgtExample}
...
**Example `r chapnum`.4.6. Portfolio Management.** You are the Chief Risk Officer of a telecommunications firm. Your firm has several property and liability risks. We will consider:
...
Denote the total risk as $X = X_1 + X_2 + X_3 + X_4$. For simplicity, you assume that these risks are independent. (Later, in Section \@ref(S:CopImp), we will consider the more complex case of dependence.)
...
```{r warning=FALSE, message=FALSE, comment="", eval=FALSE, echo=SHOW_PDF}
# For the gamma distributions, use
alpha1 <- 2;      theta1 <- 100
alpha2 <- 2;      theta2 <- 200
# For the Pareto distributions, use
